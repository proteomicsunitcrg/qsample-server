...
public class FavoriteRequestService {
    ...
    public FavoriteRequest save(FavoriteRequest favoriteRequest) {
        ...
        if (favrequest.isPresent()) {
            ...
            FavoriteRequest newRequest = new FavoriteRequest(null, favoriteRequest.getRequestCode(),
                    favoriteRequest.getAgendoId());
    ...
    public boolean checkIfFav(Long agendoId) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        User u = userRepository.findByUsername(authentication.getName()).get();
        Optional<FavoriteRequestsUsers> optReq = favoriteRequestUsersRepository
                .findOneByFavoriteRequestAgendoIdAndUser(agendoId, u);
        if (optReq.isPresent()) {
            return true;
        } else {
            return false;
    ...
    public boolean removeFav(FavoriteRequest favRequest) {
        ...
        User u = userRepository.findByUsername(authentication.getName()).get();
        Optional<FavoriteRequestsUsers> optReq = favoriteRequestUsersRepository
                .findOneByFavoriteRequestAgendoIdAndUser(favRequest.getAgendoId(), u);
        ...
        Optional<List<FavoriteRequestsUsers>> optCheckLast = favoriteRequestUsersRepository
                .findByFavoriteRequestAgendoId(favRequest.getAgendoId()); // get all the relations
        if (!optCheckLast.isPresent()) { // if no relations means that the favrequest doesnt have any user
            Optional<FavoriteRequest> favRequestOpt = favoriteRequestRepository
                    .findOneByAgendoId(favRequest.getAgendoId());
    ...
    public List<MiniRequest> getFavLocal() {
        ...
        if (favListOpt.isPresent()) {
            for (FavoriteRequestsUsers request : favListOpt.get()) {
                requests.add(
                        requestService.getLocalRequestByRequestCode(request.getFavoriteRequest().getRequestCode()));
            }

            for (RequestLocal request : requests) {
                miniRequests.add(new MiniRequest(
                        request.getId(),
    ...
    public FavoriteRequestsUsers getFavoriteRequestRelationByAgendoId(Long agendoId) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        User u = userRepository.findByUsername(authentication.getName()).get();
        Optional<FavoriteRequestsUsers> favRequestOpt = favoriteRequestUsersRepository
                .findOneByFavoriteRequestAgendoIdAndUser(agendoId, u);
